...
| 1.4 | Create `backend/src/index.ts` — bare Express app that starts on port 4000 | `[YOU]` |
...
| 3.4 | `POST /api/auth/setup-master` — store `masterSalt`, `masterVerifyToken`, `masterVerifyIv` on User | `[YOU]` |
...
| 5.2 | `lib/crypto.ts` — `encryptPassword(plaintext, key)` → `{ ciphertext, iv, authTag }` | `[CC]` |
| 5.3 | `lib/crypto.ts` — `decryptPassword(ciphertext, iv, authTag, key)` | `[CC]` |
| 5.4 | `lib/crypto.ts` — `verifyMasterPassword(enteredPassword, salt, verifyToken, verifyIv)` | `[CC]` |
...
| 6.3 | `PUT /api/passwords/:id` — update title, url, category, ciphertext, iv, authTag | `[YOU]` |
